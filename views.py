...
def homepage(request):
    ...
    if request.user.is_authenticated:
        ...
        if profile.is_company:
            gigs = Gigs.objects.filter(user = request.user).order_by('-date')
        else:
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        profile = userProfile.objects.create(user=user,phone  = request.POST.get('phone'),age = request.POST.get('age'), image = request.FILES['image'],skills = request.POST.get('skills'),location = request.POST.get('country'),is_company = False,qualification = request.POST.get('qualification'),reg_no = request.POST.get('admno'),resume = request.FILES['resume'])
        profile.save()
...
def registerNormal(request):
    ...
    if request.method == 'POST':
        ...
        profile = userProfile.objects.create(user=user,phone  = request.POST.get('phone'),age = request.POST.get('age'), image = request.FILES['image'],skills = request.POST.get('skills'),location = request.POST.get('country'),job = request.POST.get('job'),is_freelancer = False,resume = request.FILES['resume'])
        profile.save()
...
def gigView(request,pk):
    ...
    for p in prfs:
        if p.user.username != "guestuser":
            ...
            if ps.status == "":
                prof2 = {"username":p.user.username,"location":p.location,"starting":p.starting_rate,"ending":p.maximum_rate,"skills":p.skills.split(',')[:2],"image":p.image.url,"pk":p.user.pk,"job":p.job,"age":p.age,"is_company":p.is_company,"status":False,"text":ps.status,'pk':p.user.pk,"resume":p.resume}
            else:
                prof2 = {"username":p.user.username,"location":p.location,"starting":p.starting_rate,"ending":p.maximum_rate,"skills":p.skills.split(',')[:2],"image":p.image.url,"pk":p.user.pk,"job":p.job,"age":p.age,"is_company":p.is_company,"status":True,"text":ps.status,'pk':p.user.pk,"resume":p.resume}
...
def convert_pdf_to_txt(path):
    ...
    file_handle.close()
    return text
openai.api_key = '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'
import re 
def extract_skills(resume_text):
    # Define a list of predefined skills
    predefined_skills = [
        # Programming Languages
        "Python", "Java", "JavaScript", "SQL", "HTML", "CSS", "C++", "C#", "PHP", "Ruby", "Swift", "Objective-C",
        # Web Development Frameworks and Libraries
        "React", "Angular", "Vue.js", "Node.js", "Django", "Flask", "Spring", "Hibernate", "jQuery", "Bootstrap", "SASS", "LESS", "TypeScript",
        # Database Technologies
        "MongoDB", "MySQL", "PostgreSQL", "SQLite", "Firebase",
        # Version Control and DevOps
        "Git", "SVN", "Docker", "Kubernetes", "AWS", "Azure", "Google Cloud", "Heroku", "Linux", "Windows", "MacOS",
        # Mobile Development
        "iOS", "Android",
        ...
        # Business Analysis
        "Business Analysis", "Requirements Gathering", "Data Analysis", "Process Improvement", "Business Process Modeling",
        # Database Administration
        "Database Management", "SQL Server", "Oracle", "MySQL Administration", "Database Security", "Backup and Recovery"
    ]

    # Define a regular expression pattern to extract skills
    skill_pattern = r'(?:^|\b)([A-Za-z\s&]+)(?=\b)'
    skills = re.findall(skill_pattern, resume_text)

    # Match extracted skills with predefined skills
    matched_skills = [skill.strip() for skill in skills if skill.strip() in predefined_skills]
...
def upload(request):
    ...
    # Assuming the file is in PDF format, extract text from PDF
    ...
    # Predict the category based on extracted skills
    result = loaded_model.predict([" ".join(skills)])[0]

    print("Predicted category:", result)

    # Fetch relevant gigs based on predicted category
    gigs_list = []
    gigs_all = Gigs.objects.filter(category=result)

    for gig in gigs_all:
        saved = Saved.objects.filter(Q(gig_pk=gig.pk) & Q(user=request.user)).exists()
        user = User.objects.get(pk=gig.user.pk)
    ...
    for p in profiles:
        if p.user.username != "guestuser":
            prof = {
                ...
                "location": p.location,
                "starting": p.starting_rate,
                "ending": p.maximum_rate,
                "skills": p.skills.split(',')[:2],
                "image": p.image.url,
                "pk": p.user.pk,
                "job": p.job,
                "age": p.age,
